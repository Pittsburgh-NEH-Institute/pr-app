...
----
**Note:** The preceding description of how facets works addresses only the use of facets to “drill down” into a result set, that is, to fine-tune the results by progressively *eliminating* matches according to specific features. Facets can also be used to *broaden* a search incrementally by adding additional values, an approach that we implement in our Ghost Hoax app and describe in detail below.
...
Both facets and fields can index on computed values (that is, values that do not appear literally in the source XML), which, for reasons described below, can improve response time during query and retrieval. The computation can use the standard XPath and XQuery function libraries, as well as user-defined functions. Below we describe first facets and then fields, including where appropriate information about how to import user-defined functions into an index file so that they can be used during configuration.
